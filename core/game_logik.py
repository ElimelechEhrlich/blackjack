...
def run_full_game(deck: list[dict], player: dict, dealer: dict) -> None:
    ...
    player = player['hand']
    dealer = dealer['hand']
    while calculate_hand_value(player) < 21:
        choice = ask_player_action()
        if choice == 'H':
            player.append(deck.pop(0))
            player_hand_value = calculate_hand_value(player)
        elif choice == 'S':
            dealerplay = dealer_play(deck,dealer)
            dealer_hand_value = calculate_hand_value(dealer)
            if dealerplay:
                player_hand_value = calculate_hand_value(player)
                dealer_hand_value = calculate_hand_value(dealer)
                if player_hand_value > dealer_hand_value: 
                    ...
                    print |('draw!')
        print (f'len: {len(deck)}')
        print (f'player: {calculate_hand_value(player)}')
        print (f'dealer: {calculate_hand_value(dealer)}')
